...
- [ ] With P1/P3: rehearse the two-device approval handoff using the real mobile PWA route — phone provides the hotspot/local LAN, laptop joins it and runs `npm run build && npm run start:mobile`, phone opens `http://<laptop-local-ip>:3000/approve?user=<sam>`, adds it to the home screen, and keeps the approver screen open before the beat. If local-IP install/access fails, switch to the Phase-0 fallback path already chosen: same-Wi-Fi, HTTPS tunnel, or Android USB port forwarding.
